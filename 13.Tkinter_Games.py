...
def QuizGame():
    ...
    def Question(event):
        ...
        label1.update()
        # Checking the user's answer and changes his score

        time.sleep(1)

        questions.remove(questions[0])
        # Removing the previous question from the list

        if len(questions) == 0:
            End()
        # If the questions run out, the game ends
            ...
            d.destroy()
            e.destroy()
            # Destroying the question screen

            label1.configure(text=("Score", score))
            label.config(text="Press Enter to continue.")
            # Creating the screen that appears between the questions
...
def WordGame():
    ...
    def Start(event):
        ...
        # Configuring the label with a random word
...
def Replay(game):
    ...
    # The "ReplayButton" that is created calls different functions according to what game the user played last
